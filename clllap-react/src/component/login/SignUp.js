...
export default function SignUp() {
    console.log('policyStringss:',PolicyString);
    //value
    ...
    const RegisterClick = () => {
       // initError();
        ...
        if(nickname&&phoneNum&&gender&&age&&country&&area){
            ...
            serverController.connectFetchController(`user/regist`, 'POST', formData, function(res) {
                if (res.result===1) {
                    //history.push('/signup/complete');
                    alert("회원가입완료!!!");
                    if(window.ReactNativeWebView){
                        window.ReactNativeWebView.postMessage(JSON.stringify({type:"pageMove",target:"signupComplete"}))
                    }
                }else{
                    setPreventRegister(true);
                }
    ...
    return (
        <Container>
            <SignUpWrap>
                ...
                <SignUpList >
                    {/* 여기서 유효성 검사 하고 검사 통과하면 setValidate */}
                     { "user_join_check" === "n" ? null : 
                     <> 
                     <InputWrap>
                        <SignUpInput errorOn={toggle1}  type="email" placeholder="이메일 주소를 입력해주세요." onChange={(e)=>{setEmail(e.target.value)}}/>
                    </InputWrap>
                    <InputWrap>
                        <SignUpInput errorOn={toggle2}  type="password" placeholder="비밀번호를 입력해주세요." onChange={(e)=>{setPassword(e.target.value)}}/>
                    </InputWrap>
                    <InputWrap>
                        <SignUpInput errorOn={toggle3}  type="password" placeholder="비밀번호를 다시 입력해주세요." onChange={(e)=>{setCheckPwd(e.target.value)}}/>
                    </InputWrap>
                    </> 
                    }
                    <InputWrap>
                            ...
                            <MoreBtn on={termsMore} onClick={()=>{setTermsMore(!termsMore)}}>{termsMore ? '접기' : '펼쳐보기'}</MoreBtn>
                            {
                                ...
                                <MoreWrap>
                                    ...
                                    {/*<TermsSubBox>
                                        ...
                                        <MoreBtn on={termsMore} onClick={()=>{alert("마케팅")}}>[보기]</MoreBtn>
                                    </TermsSubBox>*/}
                                </MoreWrap>
                            }
                        </TermsBox>
