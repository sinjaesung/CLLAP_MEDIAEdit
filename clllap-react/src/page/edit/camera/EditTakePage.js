...
export default function EditTakePage() {
    ...
    const [takelist,setTakelist] = useState([{uri:"https://clllap.s3.ap-northeast-2.amazonaws.com/clllap/originalvideo00gJ5d0nBkjEnA9qoeFttake4.mp4",duration:70},{uri:"https://clllap.s3.ap-northeast-2.amazonaws.com/clllap/originalvideo00gJ5d0nBkjEnA9qoeFttake4.mp4",duration:8},{uri:"https://clllap.s3.ap-northeast-2.amazonaws.com/clllap/originalvideo00gJ5d0nBkjEnA9qoeFttake4.mp4",duration:22},{uri:"https://clllap.s3.ap-northeast-2.amazonaws.com/clllap/originalvideo00gJ5d0nBkjEnA9qoeFttake4.mp4",duration:30}]);
    ...
    useEffect(()=>{
        ...
        document.addEventListener("message",async (event) =>{
            ...
            if(playbarstatus){
                ...
                playstatus.current.style.left=3.333*(parseInt(playbarstatus)-1)+'%';
            }
    ...
    function video_takecontrol_move_closure(){
        ...
        /*function mousedown_module(event){
            ...
            if(move_target.className.indexOf('videotake_control')==-1){
                if(move_target.className.indexOf("videotake_original")!=-1){
                    ...
                    console.log("videotake_origial요소자체를 이동시키려는경우 이동연산만:",move_target,event.target);

                }else{
                    //좌우 리사이즈 요소 관련처리한경우
                    console.log('videoTkae_control resizeElements:',move_target,event.target);
                    move_target=event.target;
        ...
        function mouseup_module(event){
            ...
            if(isdrag){
                ...
                if(move_target.className.indexOf("videotake_original_")!=-1){
                    console.log('videotake_original요소를 이동시키려고 하는경우:',move_target);
                    ...
                    settakecontrolvalid(temp);//false,true,false이런식으로 요청한 테이크바 유효성상태등 점검.

                    rel_object = document.getElementById(`take${rel_object}_textValid`);
                    console.log('videotake original요소움직여서,valid값 관련결정:',rel_object);
                    if(rel_object){
                        rel_object.innerText='takebar를 움직여 편집할 최종상태 결정해주세요';
                        rel_object.style.color='red';
                    }
                   
                    
                    //rel_object_videotake=document.getElementById(rel_object_videotake);
                    //console.log('관련타깃>>>:',rel_object_videotake);
                    ...
                    console.log('=====>대상타깃 leftoffsetX시작위치 index값:',distance_array[0].index);
                    ...
                    //let change_duration=parseInt()
                    let adapt_endpos=adapt_startpos + original_duration;

                    //adapt_startpos=adapt_startpos < 0 ? 0: adapt_startpos;
                    //adapt_endpos = adapt_endpos > rel_object_startpos ? rel_object_startpos : adapt_endpos;
                    //adapt_endpos = adapt_endpos > 30 ? 30 : adapt_endpos;

                    //let rel_object_videotake_startpos= parseInt(rel_object_videotake.getAttribute("startpos"));
                    //let rel_object_videotake_endpos= parseInt(rel_object_videotake.getAttribute("endpos"));
                    //rel_object_videotake_startpos =rel_object_videotake_startpos < adapt_startpos ? rel_object_videotake_startpos : adapt_startpos;
                    //rel_object_videotake_startpos = adapt_startpos < 0? 0: adapt_startpos;
                    //rel_object_videotake_endpos = adapt_endpos < rel_object_videotake_
                    //console.log('===>관련 기존 takebar요소:',rel_object_videotake_startpos,rel_object_videotake_endpos);

                    rel_object_videotake_startpos = rel_object_videotake_startpos > adapt_startpos ? rel_object_videotake_startpos : adapt_startpos;
                    rel_object_videotake_startpos = rel_object_videotake_startpos < 0 ? 0: rel_object_videotake_startpos;
                    rel_object_videotake_endpos = rel_object_videotake_endpos < adapt_endpos ? rel_object_videotake_endpos : adapt_endpos;
                    rel_object_videotake_endpos = rel_object_videotake_endpos > 30 ? 30 : rel_object_videotake_endpos;
                    console.log('===>반영된 startpos,endpos값:',rel_object_videotake_startpos,rel_object_videotake_endpos);

                    //원본영상
                    move_target.setAttribute("endpos",adapt_endpos);
                    move_target.setAttribute("startpos",adapt_startpos);
                    //테이크편집컷팅요소타임라인
                    //rel_object_videotake.setAttribute("startpos",rel_object_videotake_startpos);
                    //rel_object_videotake.setAttribute("endpos",rel_object_videotake_endpos);

                    console.log('[[[[[moving adapt start~end posss]]]]]]] ((원본영상))',adapt_startpos,adapt_endpos);
                    //console.log('====>>>[[테이크편집cut bar요소]]]]:',rel_object_videotake_startpos,rel_object_videotake_endpos);

                   let take_video_process_info=[];
                   let split_and_move_video_controls=document.getElementsByClassName('take_row');
                   let original_takevideo_controls=document.getElementsByClassName("videotake_original_");//비디오편집 원본영상정보(start,endpos)
                   ...
                   for(let s=0; s<split_and_move_video_controls.length; s++){
                        if(split_and_move_video_controls[s].style.display=='block'){
                            ...
                            console.log('originlal-vidoesss:',original_videos,original_video_startpos,original_video_endpos);
                            ...
                            //store_obj['isvalid'] = false;//최종 편집판정 상태는아니다.
                    ...
                    setStandbydata({
                        ...
                        take_video_process_info : take_video_process_info //테이크편집정보만 지정합니다.
                    }) 
                }
                else if(move_target.className.indexOf('videotake_control')!=-1){
                    ...
                    //adapt_endpos= adapt_endpos >30? 30 : adapt_endpos;
                    adapt_endpos = adapt_endpos > rel_object_endpos ? rel_object_endpos: adapt_endpos;
                    adapt_endpos = adapt_endpos > 30 ? 30 : adapt_endpos;
        ...
        function touchmove_module(event){
            ...
            if(isdrag){
                ...
                console.log('touchmobrd이벤트발생 mobrd 당시떄의 offset위치정보값:',move_x,move_y);
                ...
                if(move_target.className.indexOf('videotake_original_')!=-1){
                   ...
                   // let adapt_x= move_x;
                    let distance_array=[];
                    for(let t=0; t<time_grid_elements.length; t++){
                        //console.log('===>touchmovesss당시떄의 adapt_x좌표 offsetX,clientX와 가장 가까운 대상찾기');
                        let item=time_grid_elements[t];
                        let offsetX=item.offsetLeft;
                       //console.log('item offsetX,adapt_x:',offsetX,move_x);
        ...
        function touchend_module(event){
            ...
            if(isdrag){
                ...
                if(move_target.className.indexOf('videotake_original_')!=-1){
                    ...
                    let distance_array=[];
                    for(let t=0; t<time_grid_elements.length; t++){
                        console.log('===>touchendup당시떄의 move_x offsetX,clientX와 가장 가까운 대상찾기');
                        let item=time_grid_elements[t];
                        let offsetX=item.offsetLeft;
...
const EditTakeWrap = styled.div`
    background: transparent;background-color:rgba(200,180,50,1);
...
const EditTakeContent = styled.div`
    ...
    height:auto;position:relative;height:100%;
    background: rgba(255,255,255,0.3);
...
const PlaystatusArea = styled.div`
    width:100%;height:100%;position:absolute;background-color:rgba(0,0,0,0);left:0;top:0;padding:0 calc(100vw *(38/428))
`
...
const OriginalTakeImg = styled.div`
    position:absolute;width:100%;height:calc(100vw *(50/428));bottom:0;left:0;background-color:rgba(0,0,0,1);z-index:-1;overflow:hidden;
`
