...
export default class CameraScreen extends React.Component{
 ...
 // const [reqUserid,setreqUserid] = useState(30);//요청userid값
  ...
  constructor(props){
      super(props);
      console.log('camsScreen classs consturcotr:',props);
  ...
  componentDidMount(){
      console.log('CameraScreen컴포넌트마운트시점:');
      this.props.navigation.addListenber("focus",(_)=>{
          this.pause();
          this.setActive();
      })
      initPermission();
  ...
  setActive(){
      ffprint("Camerascreen tab actiavted");
      enableLogCallback(this.logCallback);
      enableStatisticsCallback(this.statisticsCallback);
      showPopup(this.popupReference,"CAMERAN SCREEN PROCESSING");
  }
  logCallback = (log)=>{
      ffprint("cameraScreen ffmpegLogss:',log);")
  }
  ...
  showProgressDialog(){
      ...
      resetStatistics();
      this.progressModalReference.current.show("Creating process");
  }
  ...
  onPlayError = (err)=>{
      ffprint("play errorss:"+JSON.stringify(err));
  }


   initlPermission= async function (){
       console.log('camera페이지도달 권한요청:');
  ...
  saveImage= function (takeFile,s3upload_url_datagatherings){
  ...
  submitFiles= async function (){
    ...
    if(this.state.takeFileList.length==0){
      alert("촬영or영상 선택이 필요합니다.");
      return false;
    }
    for(var i=0;i<this.state.takeFileList.length;i++){
      saveImage(this.state.takeFileList[i],s3upload_url_datagatherings);
    }

    standby= async function (){
      return new Promise((resolve,reject)=>{
        let standby_cnt=0;
        let standby_interval=setInterval(function(){
          if(this.state.takeFileList.length === s3upload_url_datagatherings.length){
    ...
    return (
      ...
      <View style={styles.container}> 
        ...
        >
          {({ camera, status, recordAudioPermissionStatus }) => {
            return (
              <View>
                <CaptureBar 
                  camera={camera} 
                  takeFileList={this.state.takeFileList} 
         ...
         <CaptureTop isFront={this.state.isFront}/>
         {
           this.state.maxCount == this.state.takeFileList && this.state.takeFileList.length ? 
           <TouchableOpacity style={styles.submitContainer} onPress={this.submitFiles}>
