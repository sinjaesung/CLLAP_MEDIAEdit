...
export default function SaveScreen({navigation,route}){
    console.log('SaveScreen 페이지도달::',navigation,route);
    ...
    //let effect=route.params&& route.params.effect;
   // let overlayvideo=route.params && route.params.overlayvideo;
   ...
   // const [cuttakelist,setCuttakelist]= useState([]);
    ...
    const handleEndLoading= async(e)=>{
      ...
      webviewRef.current.postMessage(JSON.stringify({"nextdatas":nextdatas}));
    ...
    function  onMessage( event ){
      ...
      //if(data.type == "CAMERA" || data.type == "GALLERY" ){
        navigation.navigate(
          "Share",
          {
            nextdatas:data.nextParam['nextdatas'],
          })
      //}
   }
    ...
    return ( 
        ...
        <WebView 
            source={{ uri: `http://192.168.25.40:3000/save` }}
            ref={webviewRef}
