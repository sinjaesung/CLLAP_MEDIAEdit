...
export default class PipeTab extends React.Component {
    ...
    logCallback = (log) => {
        console.log('pipeTabs logCallbacksss:',log);
        ffprint(log.message);
    }

    statisticsCallback = (statistics) => {
        console.log('pipeTabs statisticsCallbacksss:',statistics);
        this.setState({statistics: statistics});
        this.updateProgressDialog();
    }

    asyncAssetWriteToPipe(assetName, pipePath) {
        console.log('AsytncAssetWriteToPiepss:',assetName,pipePath);
                ...
                registerNewFFmpegPipe().then((pipe3) => {
                    ...
                    executeFFmpegAsync(ffmpegCommand, async(completedExecution) => {
                            ...
                            if (completedExecution.returnCode === 0) {
                                ...
                                console.log('playing videoFilesss ffmpeg processreuslts filess:',videoFile);
                                
                                this.playVideo();

                                let results=await CameraRoll.save(videoFile);
                                console.log('pipe test processing resulst files to app cacheDatas to app external gallery default save',results);
                            } else {
    ...
    updateProgressDialog() {
        ...
        console.log('updateProgressDialogsss:',statistics);
